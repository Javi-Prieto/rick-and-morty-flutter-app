      ...
      listener: (context, state){
        if(state is EpisodesDetailClick){
          Navigator.push(context, 
            CupertinoPageRoute(
              builder: (context) => EpisodeDetailScreen(episodeId: state.episodeId)
            )
          ).then((value) => {EpisodesBloc(episodeRepository).add(EpisodesFetchEvent())});
        }
      },
      ...
      itemBuilder: (BuildContext context, int index){
        return SizedBox(
          ...
          child: InkWell(
            ...
            onTap: (){
              BlocProvider.of<EpisodesBloc>(context).add(EpisodesGoDetailEvent(episodeId: episodes[index].id!));
            },
          ),
        );
      },
